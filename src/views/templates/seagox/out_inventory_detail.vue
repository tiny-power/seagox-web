        ...
        <div>
            ...
            <el-form size="small">
                ...
                >
                    ...
                    <el-table-column align="center" label="出库数量" prop="cksl">
                        <template slot-scope="scope">
                            <el-form-item>
                                <el-input-number
                                    v-model="scope.row.cksl"
                                    :controls="false"
                                    :min="1"
                                    :precision="0"
                                    placeholder="请输入出库数量"
...
export default {
    ...
    methods: {
        ...
        queryOrderData() {
            this.$axios.get('cloud/inventory/queryList').then(res => {
                if (res.data.code == 200) {
        ...
        async queryDetailData() {
            let params = {
                outInventoryId: this.$router.currentRoute.query.id
            }
            let res = await this.$axios.get('cloud/outInventoryDetail/queryDetailByInventoryId', { params })
            if (res.data.code == 200) {
