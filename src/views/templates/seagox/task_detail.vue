<template>
    <div style="background: white; width: 100%">
        ...
        <el-dialog title="选择" width="550px" :visible.sync="formVisible" :close-on-click-modal="false">
            <el-table
                :data="productData"
                border
                style="width: 100%"
...
export default {
    ...
    methods: {
        ...
        async queryDetailData() {
            let params = {
                taskId: this.$router.currentRoute.query.id
            }
            let res = await this.$axios.get('cloud/task/queryDetailByTaskId', { params })
            if (res.data.code == 200) {
        ...
        valid() {
            let validFlag = false
            if (this.$parent.state.type.value == 1) {
