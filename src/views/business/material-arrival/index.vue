    ...
    <div class="document-page">
            ...
            >
                ...
                <el-table-column
                    ...
                    show-overflow-tooltip
                >
                    <template slot-scope="scope">{{ formatValue(scope.row.name) }}</template>
                </el-table-column>
                <el-table-column
                    prop="arrivalAt"
                    label="到场时间"
                    ...
                    show-overflow-tooltip
                >
                    <template slot-scope="scope">{{ formatValue(scope.row.arrivalAt) }}</template>
                </el-table-column>
                <el-table-column
                    prop="createdByName"
                    label="登记人"
...
export default {
    ...
    methods: {
        ...
        async load() {
            this.loading = true
            try {
                let response = await this.$axios.get('materialArrival/queryByPage', { params: this.buildParams() })
                if (response.data.code === 200) {
                    let data = response.data.data || {}
                    this.rows = data.list || []
        ...
        view(row) {
            this.$router.push({ path: '/materialArrivalDetail', query: { id: row.id } })
        }
    }
}
</script>
