...
export default {
    ...
    methods: {
        ...
        parseSummary(value) {
            if (!value) return { delta: emptyDelta(), html: '' }
            if (typeof value === 'object') return { delta: value.delta || emptyDelta(), html: value.html || '' }
            try {
                const data = JSON.parse(value)
                return { delta: data.delta || emptyDelta(), html: data.html || '' }
            } catch (error) {
                return { delta: emptyDelta(), html: value }
            }
        },
        async load() {
            ...
            try {
                ...
                if (response.data.code === 200) {
                    ...
                    this.form = {
                        ...
                        summary: this.parseSummary(data.summary),
                        styleList: this.parseArray(data.styleList),
                        regionText: data.regionText,
                        provinceName: data.provinceName,
                        cityName: data.cityName,
                        buildingArea: data.buildingArea === null ? undefined : Number(data.buildingArea),
                        floorCount: data.floorCount === null ? undefined : Number(data.floorCount),
                        roofType: data.roofType,
                        specialList: this.parseArray(data.specialList),
                        status: Number(data.status),
                        sort: Number(data.sort || 0)
                    }
                    this.coverFileList = data.coverUrl ? [{ name: data.title || '封面图片', url: data.coverUrl }] : []
        ...
        submit() {
            this.$refs.form.validate(async valid => {
                ...
                this.saving = true
                try {
                    const response = await this.$axios.post('designCase/update', {
                        ...this.form,
