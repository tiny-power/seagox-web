<template>
    <div class="document-page">
        <el-card shadow="never">
            <div slot="header" class="card-header"><span>验收单</span></div>
            ...
            >
                ...
                <el-table-column
                    ...
                    show-overflow-tooltip
                >
                    <template slot-scope="scope">{{ formatValue(scope.row.planInspectionTime) }}</template>
                </el-table-column>
                <el-table-column
                    prop="passedAt"
                    label="通过时间"
                    ...
                    show-overflow-tooltip
                >
                    <template slot-scope="scope">{{ formatValue(scope.row.passedAt) }}</template>
                </el-table-column>
                <el-table-column
                    prop="status"
                    label="状态"
                    ...
                    show-overflow-tooltip
                >
                    <template slot-scope="scope">{{ formatInspectionStatus(scope.row.status) }}</template>
                </el-table-column>
                <el-table-column
                    prop="createdAt"
                    label="创建时间"
...
export default {
    ...
    methods: {
        ...
        async load() {
            this.loading = true
            try {
                let response = await this.$axios.get('inspection/queryByPage', { params: this.buildParams() })
        ...
        view(row) {
            this.$router.push({ path: '/inspectionDetail', query: { id: row.id } })
        }
    }
}
</script>
